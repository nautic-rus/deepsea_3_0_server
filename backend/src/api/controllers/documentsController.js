...
class DocumentsController {
  ...
  static async listWatchers(req, res, next) {
    try {
      const actor = req.user || null;
      const id = parseInt(req.params.id, 10);
      const rows = await EntityWatchersService.listWatchers('document', Number(id), actor);
      res.json({ data: rows });
    } catch (err) { next(err); }
  }
  ...
  static async addWatcher(req, res, next) {
    try {
      ...
      const created = await EntityWatchersService.addWatcher('document', Number(id), watcherIds, actor);
      res.status(201).json({ data: created });
    } catch (err) { next(err); }
  }
  ...
  static async addSelfWatcher(req, res, next) {
    try {
      const actor = req.user || null;
      const id = parseInt(req.params.id, 10);
      const created = await EntityWatchersService.addSelfWatcher('document', Number(id), actor);
      res.status(201).json({ data: created });
    } catch (err) { next(err); }
  }
  ...
  static async removeWatcher(req, res, next) {
    try {
      ...
      const ok = await EntityWatchersService.removeWatcherByUserOrWatcherId('document', Number(id), userId, watcherId, actor);
      res.json(ok);
    } catch (err) { next(err); }
  }
  ...
  static async removeSelfWatcher(req, res, next) {
    try {
      const actor = req.user || null;
      const id = parseInt(req.params.id, 10);
      const ok = await EntityWatchersService.removeSelfWatcher('document', Number(id), actor);
      res.json(ok);
    } catch (err) { next(err); }
  }
