...
class CustomerQuestionsController {
  ...
  static async listFiles(req, res, next) {
    try {
      ...
      const rows = await CustomerQuestionsService.listQuestionFiles(Number(id), { limit: limit != null ? Number(limit) : undefined, offset: Number(offset) }, actor);
      res.json({ data: rows });
    } catch (err) { next(err); }
  }
  ...
  static async listWatchers(req, res, next) {
    try {
      const actor = req.user || null;
      const id = parseInt(req.params.id, 10);
      const rows = await EntityWatchersService.listWatchers('customer_question', Number(id), actor);
      res.json({ data: rows });
    } catch (err) { next(err); }
  }
  ...
  static async addWatcher(req, res, next) {
    try {
      ...
      const created = await EntityWatchersService.addWatcher('customer_question', Number(id), watcherIds, actor);
      res.status(201).json({ data: created });
    } catch (err) { next(err); }
  }
  ...
  static async addSelfWatcher(req, res, next) {
    try {
      const actor = req.user || null;
      const id = parseInt(req.params.id, 10);
      const created = await EntityWatchersService.addSelfWatcher('customer_question', Number(id), actor);
      res.status(201).json({ data: created });
    } catch (err) { next(err); }
  }
  ...
  static async removeWatcher(req, res, next) {
    try {
      ...
      const ok = await EntityWatchersService.removeWatcherByUserOrWatcherId('customer_question', Number(id), userId, watcherId, actor);
      res.json(ok);
    } catch (err) { next(err); }
  }
  ...
  static async removeSelfWatcher(req, res, next) {
    try {
      const actor = req.user || null;
      const id = parseInt(req.params.id, 10);
      const ok = await EntityWatchersService.removeSelfWatcher('customer_question', Number(id), actor);
      res.json(ok);
    } catch (err) { next(err); }
  }
  ...
  static async listMessages(req, res, next) {
    try {
      ...
      const id = parseInt(req.params.id, 10);
      const { limit, offset = 0 } = req.query || {};
      const rows = await CustomerQuestionsService.listQuestionMessages(Number(id), { limit: limit != null ? Number(limit) : undefined, offset: Number(offset) }, actor);
      res.json({ data: rows });
    } catch (err) { next(err); }
  }
