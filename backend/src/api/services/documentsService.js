...
class DocumentsService {
  ...
  static async _attachRevisionAndFileFlagsToList(rows, logPrefix = 'documents list') {
    try {
      ...
      const statsQ = `
        SELECT
          document_id,
          ARRAY_AGG(DISTINCT rev) FILTER (WHERE archive = false) AS revs,
          COUNT(*)::int AS attach_count
        FROM documents_storage
        WHERE document_id = ANY($1::int[])
        GROUP BY document_id
      `;
      const statsRes = await pool.query(statsQ, [docIds]);
      ...
      for (const r of (statsRes.rows || [])) {
        ...
        if (arr.length === 0) {
          ...
          if (!isNumeric && !isAlpha) {
            ...
            revision = hasNumeric && hasAlpha ? '-' : '-';
          } else if (isNumeric) {
            revision = Math.max(...arr.map(Number));
          } else if (isAlpha) {
            arr.sort((a, b) => a.localeCompare(b, 'en', { sensitivity: 'base' }));
            revision = arr[arr.length - 1];
          } else {
            revision = '-';
          }
        }
        ...
        statsMap.set(id, {
          revision,
          have_attach: Number(r.attach_count) > 0
        });
      }

      for (const it of rows) {
        const stats = statsMap.get(Number(it.id));
        it.revision = stats ? stats.revision : null;
        it.have_attach = stats ? stats.have_attach : false;
      }
    } catch (e) {
      console.error(`Failed to attach revision/file flags to ${logPrefix}`, e && e.message ? e.message : e);
    }
  }
  static async listDocuments(query = {}, actor) {
    ...
    if (!permissionScope.hasGlobal && permissionScope.projectIds.length === 0) {
      const err = new Error('Forbidden: missing permission documents.view'); err.statusCode = 403; throw err;
    }
    ...
    if (canViewAll || permissionScope.hasGlobal) {
      ...
      await DocumentsService._attachRevisionAndFileFlagsToList(rows, 'documents list');
      return rows;
    }

    // Otherwise restrict results to projects where user has documents.view.
    ...
    // If the query specifies project_id, ensure all requested ids are permitted.
    if (query.project_id !== undefined && query.project_id !== null) {
      ...
      if (forbiddenProject !== undefined) {
        const err = new Error('Forbidden: missing permission documents.view for requested project'); err.statusCode = 403; throw err;
      }
    ...
    await DocumentsService.attachDisplayFieldsToList(rows);
    await DocumentsService._attachRevisionAndFileFlagsToList(rows, 'documents list (restricted)');
    return rows;
  }
  ...
  it.assigne_to = assignee
    ? {
        id: assignee.id,
        full_name: mkUserDisplay(assignee),
        avatar_id: assignee.avatar_id || null
      }
    : null;
      }
    } catch (e) {
  ...
  static async createDocument(fields, actor) {
    ...
    const allowed = await hasPermissionForProject(actor, requiredPermission, project.id);
    if (!allowed) { const err = new Error('Forbidden: missing permission documents.create for target project'); err.statusCode = 403; throw err; }
  ...
  static async updateDocument(id, fields, actor) {
    ...
    if (!allowedCurrent) { const err = new Error('Forbidden: missing permission documents.update for this project'); err.statusCode = 403; throw err; }
    ...
    if (fields.project_id !== undefined && fields.project_id !== null && Number(fields.project_id) !== Number(existing.project_id)) {
      ...
      if (!allowedTarget) { const err = new Error('Forbidden: missing permission documents.update for target project'); err.statusCode = 403; throw err; }
    }
  ...
  static async deleteDocument(id, actor) {
    ...
    const allowed = existing.project_id
      ? await hasPermissionForProject(actor, requiredPermission, existing.project_id)
      : await hasPermission(actor, requiredPermission);
    if (!allowed) { const err = new Error('Forbidden: missing permission documents.delete for this project'); err.statusCode = 403; throw err; }
