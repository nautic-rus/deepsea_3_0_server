...
class UserProjectsService {
  ...
  static async listByProject(projectId, actor) {
    ...
    const q = `SELECT id, user_id, project_id, role, assigned_by, assigned_at FROM user_projects WHERE project_id = $1 ORDER BY assigned_at DESC`;
    const res = await require('../../db/connection').query(q, [projectId]);
    return res.rows;
  }

  // Assign a user to a project (actor must have permission or be owner)
  static async assignToProject(projectId, userId, role, actor) {
    if (!actor || !actor.id) { const err = new Error('Authentication required'); err.statusCode = 401; throw err; }
    ...
    return await UserProject.assign(userId, projectId, role || null, actor.id);
  }
