...
class SpecializationsService {
  static async list(actor) {
    const requiredPermission = 'specializations.view';
    if (!actor || !actor.id) { const err = new Error('Authentication required'); err.statusCode = 401; throw err; }
    const allowed = await hasPermission(actor, requiredPermission);
    if (!allowed) { const err = new Error('Forbidden: missing permission specializations.view'); err.statusCode = 403; throw err; }
