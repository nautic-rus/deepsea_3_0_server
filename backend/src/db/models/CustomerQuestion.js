  ...
  static async list(filters = {}) {
    ...
    const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
        let q = `SELECT cq.id, cq.project_id, cq.question_title, cq.question_text, cq.answer_text, cq.comment, cq.priority,
           ...
           cq.status_id, cs.name AS status_name, cs.code AS status_code, cs.color AS status_color, cs.description AS status_description,
    ...
    return rows.map(r => ({
      ...
      status: r.status_id ? { status_id: r.status_id, status_code: r.status_code, status_name: r.status_name, status_color: r.status_color } : null,
