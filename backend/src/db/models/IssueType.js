...
class IssueType {
  static async list(projectId) {
    if (typeof projectId !== 'undefined') {
      const q = 'SELECT * FROM issue_type WHERE (project_id IS NULL OR project_id = $1) ORDER BY name, id';
      const res = await pool.query(q, [projectId]);
      return res.rows || [];
    }
    const res = await pool.query('SELECT * FROM issue_type ORDER BY name, id');
    return res.rows || [];
  }
