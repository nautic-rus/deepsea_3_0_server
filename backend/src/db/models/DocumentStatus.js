...
class DocumentStatus {
  static async list(projectId) {
    if (typeof projectId !== 'undefined') {
      const q = 'SELECT * FROM document_status WHERE (project_id IS NULL OR project_id = $1) ORDER BY weight, id';
      const res = await pool.query(q, [projectId]);
      return res.rows || [];
    }
    const res = await pool.query('SELECT * FROM document_status ORDER BY weight, id');
    return res.rows || [];
  }
  ...
  static async update(id, fields) {
    ...
    if (parts.length === 0) return await DocumentStatus.findById(id);
