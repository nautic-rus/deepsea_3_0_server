...
class WikiSection {
  static async list(filters = {}) {
    const { id, parent_id, name, slug, created_by, page = 1, limit } = filters;
    ...
    let q = `SELECT id, name, slug, description, parent_id, order_index, created_by, updated_by, created_at, updated_at FROM wiki_sections ${whereSql} ORDER BY order_index, id`;
    if (limit != null) {
  ...
  static async findById(id) {
    const q = `SELECT id, name, slug, description, parent_id, order_index, created_by, updated_by, created_at, updated_at FROM wiki_sections WHERE id = $1 LIMIT 1`;
  ...
  static async update(id, fields) {
    ...
    const q = `UPDATE wiki_sections SET ${parts.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${idx} RETURNING id, name, slug, description, parent_id, order_index, created_by, updated_by, created_at, updated_at`;
    values.push(id);
    const res = await pool.query(q, values);
    return res.rows[0] || null;
  }
